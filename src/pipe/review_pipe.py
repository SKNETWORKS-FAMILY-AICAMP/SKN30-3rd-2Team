...
def review_contract(
    ...
    *,
    ...
    Args:
        ...
        top_k: 컬렉션별 검색 및 재정렬 시 가져올 상위 후보 수.
        ...
        coverage_threshold: 표준 서브청크가 '커버됨'으로 인정할 최소 유사도(0~1, sigmoid 스케일).
    ...
    for clause, std_raw, sub_raw, toxic_raw in zip(clauses, std_batch, sub_batch, toxic_batch):
        ...
        # ── 커버리지 체크: NONE 판정된 거대 조항에서 표준 항 미커버 탐지 → CHANGED 상향 ──
        ...
        if (
            ...
            if len(std_subs) >= 2:
                ...
                std_ids = [s.sub_chunk_id for s in std_subs]
                # M×N 유사도 매트릭스: reranker.compute_scores 재사용 (추가 포트 주입 없음)
                # 각 행 i = 표준 항 i 와 사용자 항 N개 간의 sigmoid 변환 점수
                matrix = [
                    [sigmoid(raw) for raw in reranker.compute_scores(s.text, user_sub_texts)]
                    for s in std_subs
                ]
                covered, uncovered_ids = check_coverage(std_ids, matrix, coverage_threshold)
                if not covered:
