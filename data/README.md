...
| `03_normalized/` | **정규화된 조항(정답).** `StandardClause` / `ClauseRelation` / `ToxicPatternRecord` 스키마를 따르는 JSON | ✅ 커밋 |
| `migration/01.CREATE_TABLE.sql` | DB 스키마(DDL) | ✅ 커밋 |
| `migration/*.sqlite3` | **생성물.** normalize 에서 빌드된 SQLite | ❌ gitignore |
| `migration/<uuid>/`, `chroma_meta.sqlite3` | **생성물.** Chroma 벡터 인덱스 | ❌ gitignore |
...
| 파일 | 모델 | 담당 |
| --- | --- | --- |
| `standard_clauses.<contract_type>.json` | `StandardClause[]` | 표준조항 정규화 담당 |
| `clause_relations.json` | `ClauseRelation[]` (고도화 A) | 도메인 큐레이션 |
| `toxic_patterns.json` | `ToxicPatternRecord[]` (고도화 B) | 도메인 큐레이션 |

> 새 계약 종류를 추가할 때는 **코드를 고치지 않고** `standard_clauses.<new_type>.json` 파일과 `ContractType` enum 값 하나만 추가하면 됩니다. (기획서 3.3 확장성)
